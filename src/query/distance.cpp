...
float asymmetric_ip_score_scalar(std::span<const float> query_rotated,
  ...
  for (std::size_t b = 0; b < dim; ++b) {
    ...
    score += query_rotated[b] * codebook.centroid_at(code);
  }
  return score;
}
...
/// bits=8: table[k] = q0 * c[k].
void fill_group_lut_bits8(float* table, float q0, const float* centroids) {
...
/// bits=4: table[b] = q0*c[lo] + q1*c[hi].
void fill_group_lut_bits4(float* table, float q0, float q1, const float* centroids) {
...
void build_query_lut(std::span<const float> query_rotated,
  ...
  const std::size_t num_groups = dim / codes_per_group;
  out.resize(num_groups, bits, dim);

  const auto centroids = codebook.centroids();
  for (std::size_t g = 0; g < num_groups; ++g) {
    const std::size_t q_off = g * codes_per_group;
    const auto q_slice = query_rotated.subspan(q_off, codes_per_group);
    if (bits == 8) {
      fill_group_lut_bits8(out.table(g), q_slice[0], centroids.data());
    } else if (bits == 4) {
      fill_group_lut_bits4(out.table(g), q_slice[0], q_slice[1], centroids.data());
    } else {
      fill_group_lut(out.table(g), q_slice, centroids, bits, codes_per_group);
    }
  }

  if (bits == 1 && (dim % 64) == 0 && codebook.num_centroids() >= 2) {
    AlignedVector<float> delta(dim);
    float base = 0.0f;
    const float c0 = codebook.centroid_at(0);
    const float c1 = codebook.centroid_at(1);
    for (std::size_t b = 0; b < dim; ++b) {
      const float q = query_rotated[b];
      base += q * c0;
      delta[b] = q * (c1 - c0);
    }
...
void asymmetric_ip_batch_lut(const QueryLut& lut, std::span<const std::uint64_t> data_words,
  ...
  if (lut.has_bit1_deltas() && lut.bits() == 1 && lut.num_codes() == dim) {
