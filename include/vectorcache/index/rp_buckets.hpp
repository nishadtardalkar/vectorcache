...
class ClusterCentroids {
 public:
  ...
  /// Then one simultaneous frame-potential step (drive pairwise ⟨ĉ_i,ĉ_j⟩→0) with step `ortho_eta`
  /// (`0` = skip). `vectors` is row-major N * dim; `cell_keys` length N is overwritten.
