...
/// Stores L1 + L0 bitcodes only; padded_dim is metadata for bit counts.
struct BlockLayout {
...
/// True when L1/L0 bit counts are multiples of 64 (SIMD-friendly).
bool layout_is_simd_friendly(const BlockLayout& layout);

/// Single-buffer storage for up to BLOCK_SIZE vectors across L1 and L0 tiers.
class LogicalBlock {
 public:
